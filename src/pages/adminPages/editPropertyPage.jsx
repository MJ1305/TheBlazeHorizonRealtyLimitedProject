...
export default function EditPropertyPage() {
  ...
  useEffect(() => {
    // TODO: fetch property from supabase by id when network is fixed
    // For now we just pre-fill with mock data to show the UI
    setForm({
      title: "The Monarch Suite",
      slug: "the-monarch-suite",
      description: "Luxury waterfront apartment in Banana Island.",
      location: "Banana Island, Lagos",
      full_address: "123 Banana Island Way, Ikoyi, Lagos",
      state: "Lagos",
      country: "Nigeria",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // TODO: update in supabase when network is fixed
  ...
  const handleQuickStatus = async (newStatus) => {
    ...
    // TODO: update status in supabase when network is fixed
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <Field key={f.name} label={f.label}>
                  <Input className={styledInput} type={f.type} name={f.name} value={form[f.name]} onChange={handleChange} />
                </Field>
              ))}
            </div>
            <FieldGroup>
          ...
          >
            <FieldGroup>
              <Field label="Price (₦)">
                <Input className={styledInput} type="number" name="price" value={form.price} onChange={handleChange} placeholder="Leave empty to hide price" />
              </Field>
          ...
          >
            ...
            <div className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-wide" style={{ color: "#1B3A2D" }}>Gallery Images</p>
              <label className="flex flex-col items-center justify-center w-full h-24 rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 cursor-pointer hover:border-green-800 transition-colors">
