...
export default function AddPropertyPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Property saved! (Supabase insert pending network fix)");
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <Field key={f.name} label={f.label}>
                  <Input className={styledInput} type={f.type} name={f.name} value={form[f.name]} onChange={handleChange} placeholder={f.placeholder} required={f.name !== "year_built"} />
                </Field>
              ))}
            </div>
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <label className="flex flex-col items-center justify-center w-full h-24 rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 cursor-pointer hover:border-green-800 transition-colors">
                <div className="text-center">
                  <p className="text-sm text-gray-400">Click to upload multiple images</p>
