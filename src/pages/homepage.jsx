...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchFeatured = async () => {
      ...
      if (!error && data) {
        // Normalize Supabase fields to match PropertyCard expectations
        const normalized = data.map((p) => ({
          ...p,
          image: p.cover_image,
          images: [p.cover_image, ...(p.images || [])].filter(Boolean),
          beds: p.bedrooms,
          baths: p.bathrooms,
          sqft: p.area_sqft || '—',
  ...
  const whyChooseUs = [
    { title: 'Expert Team', description: 'Years of combined experience in real estate', icon: '👥', color: 'bg-brand-yellow/10' },
    { title: 'Best Locations', description: 'Prime properties in premium neighborhoods', icon: '📍', color: 'bg-[#fa8e12]/10' },
    { title: 'Transparent Process', description: 'No hidden fees, clear documentation', icon: '🔍', color: 'bg-brand-yellow/10' },
    { title: '24/7 Support', description: 'Round-the-clock customer service', icon: '🕐', color: 'bg-[#03302b]/5' },
    { title: 'Best Prices', description: 'Competitive rates and flexible payment plans', icon: '💰', color: 'bg-brand-yellow/10' },
    { title: 'Legal Assurance', description: 'Proper documentation and title verification', icon: '⚖️', color: 'bg-[#03302b]/5' }
  ]
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6 lg:gap-8">
            {whyChooseUs.map((item, index) => (
              <div key={index} className="group bg-white p-4 sm:p-5 md:p-6 lg:p-8 rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-2 cursor-pointer">
                ...
                <h3 className="text-base sm:text-lg md:text-xl font-black text-[#03302b] mb-1.5 sm:mb-2 md:mb-3 transition-all duration-300 group-hover:text-[#fa8e12]">
                  {item.title}
                </h3>
                <p className="text-[#03302b]/60 leading-relaxed text-xs sm:text-sm md:text-base">{item.description}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col sm:flex-row justify-between items-center sm:items-end mb-6 sm:mb-8 md:mb-10 lg:mb-12 gap-3 sm:gap-4 md:gap-6">
            <div className="text-center sm:text-left">
              <span className="text-[#fa8e12] text-[10px] sm:text-[11px] md:text-[13px] lg:text-[15px] font-black tracking-[0.3em] uppercase mb-1 sm:mb-2 md:mb-4 block">
                New Arrivals
              </span>
          ...
          {featuredProperties.length === 0 ? (
            <div className="text-center py-16 text-gray-400 text-sm">Loading latest properties...</div>
          ) : (
            <div className="flex overflow-x-auto gap-3 sm:gap-4 md:gap-5 lg:gap-6 pb-4 sm:pb-6 md:pb-8 lg:pb-10 -mx-4 sm:-mx-6 px-4 sm:px-6 snap-x snap-mandatory lg:grid lg:grid-cols-3 lg:overflow-visible">
              {featuredProperties.map((property) => (
                <div key={property.id} className="min-w-[260px] sm:min-w-[280px] md:min-w-[300px] lg:min-w-0 snap-start">
                  <PropertyCard
                    {...property}
      ...
      <section id="faq-section" className="py-10 sm:py-14 md:py-18 lg:py-24 px-4 sm:px-6 bg-white">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-2 sm:space-y-3 md:space-y-4">
            {faqs.map((faq, index) => (
              <div key={index} className="border border-gray-200 rounded-lg sm:rounded-xl md:rounded-2xl overflow-hidden bg-white hover:shadow-lg transition-shadow duration-300">
                <button
                ...
                >
                  <span className="font-black text-[#03302b] text-xs sm:text-sm md:text-base lg:text-lg pr-2 flex-1">{faq.question}</span>
                  <span className="text-[#fa8e12] text-lg sm:text-xl md:text-2xl flex-shrink-0">{openFaq === index ? '−' : '+'}</span>
                </button>
                <div className={`transition-all duration-300 ease-in-out ${openFaq === index ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>
                  <div className="px-3 sm:px-4 md:px-6 pb-3 sm:pb-4 md:pb-6 pt-1 sm:pt-2">
                    <p className="text-[#03302b]/70 leading-relaxed text-xs sm:text-sm md:text-base">{faq.answer}</p>
                  </div>
                </div>
              </div>
            ))}
